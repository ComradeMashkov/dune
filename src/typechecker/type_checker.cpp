...
void TypeChecker::collect_function(const Statement& statement) {
    if (statement.is_foreknown && statement.is_extern) {
        throw DiagnosticError(statement.location, "foreknown functions cannot be foreign");
    }
...
void TypeChecker::collect_generic_function(const Statement& statement) {
    if (statement.is_foreknown) {
        throw DiagnosticError(statement.location, "generic foreknown functions are not supported yet");
    }
...
void TypeChecker::check_statement(const Statement& statement) {
    ...
    case StatementKind::const_statement: {
        if (statement.is_foreknown && (current_function_ != nullptr || scopes_.size() > 1)) {
            throw DiagnosticError(statement.location, "foreknown constants are only allowed at top level");
        }
...
void TypeChecker::check_foreknown_statement(const Statement& statement, std::unordered_set<std::string>& locals) const {
    ...
    case StatementKind::assign:
        if (statement.target == nullptr || statement.target->kind != ExpressionKind::identifier) {
            throw DiagnosticError(statement.location, "foreknown functions cannot mutate aggregate values");
        }
        if (!locals.contains(statement.target->lexeme)) {
            throw DiagnosticError(statement.location, "foreknown functions can only assign local values");
        }
        require_foreknown_expression(*statement.expression, locals);
        return;
    case StatementKind::block: {
    ...
    case StatementKind::expression_statement:
        ...
        return;
    case StatementKind::for_in_statement:
        throw DiagnosticError(statement.location, "foreknown functions do not support for-in yet");
    case StatementKind::function:
    case StatementKind::method_block:
    case StatementKind::struct_statement:
    case StatementKind::enum_statement:
    ...
    case StatementKind::module_declaration:
    case StatementKind::test_block:
        throw DiagnosticError(statement.location, "statement is not allowed in a foreknown function");
    }
}
...
void TypeChecker::require_foreknown_expression(const Expression& expression,
    ...
    case ExpressionKind::identifier:
        ...
        throw DiagnosticError(expression.location, "identifier '" + expression.lexeme + "' is not foreknown");
    case ExpressionKind::member:
        ...
        throw DiagnosticError(expression.location, "member expression is not foreknown");
    case ExpressionKind::unary:
        require_foreknown_expression(*expression.right, locals);
        return;
    case ExpressionKind::try_expression:
        throw DiagnosticError(expression.location, "try expressions are not foreknown");
    case ExpressionKind::cast:
        require_foreknown_expression(*expression.left, locals);
        return;
    case ExpressionKind::binary:
        if (expression.lexeme == "in") {
            throw DiagnosticError(expression.location, "operator 'in' is not foreknown");
        }
        require_foreknown_expression(*expression.left, locals);
        require_foreknown_expression(*expression.right, locals);
        return;
    ...
    case ExpressionKind::method_call: {
        ...
        if (resolved == resolved_calls_.end() || !is_foreknown_function_key(resolved->second) ||
            ...
            throw DiagnosticError(expression.location, "method call is not foreknown");
        }
    ...
    case ExpressionKind::slice:
        throw DiagnosticError(expression.location, "expression is not supported in foreknown context");
    }
}
...
bool TypeChecker::is_io_builtin(const std::string& name) {
    return name == "__read_file" || name == "__write_file" || name == "__stdout_write" ||
           ...
           name == "__plot_show_native" || name == "__canvas_show_native";
}
...
Type TypeChecker::check_io_builtin_call(const Expression& expression) {
    ...
    } else if (name == "__write_file" || name == "__canvas_show_native") {
