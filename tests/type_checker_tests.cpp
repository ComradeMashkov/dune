...
int main() {
    ...
    passed = expect_error_contains("for value in 42 { print(value); }", "type 'int' is not iterable",
             ...
             passed;
    passed = expect_error_contains("import dict; scores: dict.Dict<int> = dict.Dict.new(); "
                                   "for entry in scores { print(entry); }",
    ...
    passed = expect_valid("evens: [int] = [i for i in 0..10 if i % 2 == 0]; print(evens.len());",
             ...
             passed;
    passed = expect_valid("import set; seen: set.Set = set.Set.new(); seen.add(\"a\"); "
                          "values: [text] = [value for value in seen if value.len() > 0]; print(values.len());",
