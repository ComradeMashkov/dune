...
- `fn next_int(): int` — Advance the generator and return the raw state in [1, 2147483646]. — e.g. `random.seed(1).next_int()`
- `fn next_real(): real64` — Uniform real64 in [0.0, 1.0). Never returns exactly 0.0 or 1.0.
- `fn between(lo: int, hi: int): int` — Uniform integer in [lo, hi); `hi` is exclusive. — e.g. `random.seed(42).between(1, 10)`
- `fn real_between(lo: real64, hi: real64): real64` — Uniform real64 in [lo, hi). — e.g. `random.seed(42).real_between(0.0, 1.0)`
- `fn normal(mean: real64, stddev: real64): real64` — One sample from a normal distribution via the Box-Muller transform. — e.g. `random.seed(42).normal(0.0, 1.0)`
